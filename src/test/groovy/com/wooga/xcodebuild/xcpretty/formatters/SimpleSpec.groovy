...
class SimpleSpec extends FormatterSpec<Simple> {
    ...
    def "formats compiling storyboard output"() {
        expect:
        formatter.formatCompileStoryboard("Main.storyboard", 'path/to/file') == "> Compiling Main.storyboard"
    }
    ...
    @Unroll
    def 'formats test with colored time when colorize is true'() {
        unicodeFormatter.formatMeasuringTest("RACCommandSpec", "_tupleByAddingObject__should_add_a_non_nil_object", time) == "    T _tupleByAddingObject__should_add_a_non_nil_object measured (${expectedTimeFormat} seconds)"

        where:
        time    | expectedTimeFormat
        "0.001" | "0.001"
        "0.025" | "0.025"
        "0.026" | unicodeFormatter.yellow("0.025")
        "0.099" | unicodeFormatter.yellow("0.099")
        "0.100" | unicodeFormatter.red("0.100")
    }
