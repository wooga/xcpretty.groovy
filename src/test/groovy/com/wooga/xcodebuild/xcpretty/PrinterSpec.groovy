...
class PrinterSpec extends Specification {
    ...
    def "prints value to provided writer"() {
        when:
        printer.prettyPrint("hey ho let's go")
    ...
    def "xcpretty printer is a Writer"() {
        given:
        def text = """
        hey ho let's go
        flip flop flup
        ...
        then:
        outputWriter.toString() == text + "\n"

        cleanup:
        printer.close()
    }

    def "writes to stdout when no writer is passed"() {
        given:
        ...
        and: "a printer without writer passed"
        def stdPrinter = new Printer(new DummyFormatter())
